...
def update_generator(pDialog):
    ...
    if status == '':
        status = "BBTS list updated. if no changes are visible Please exit and re-open."
    else:
        status = "BBTS Error: " + status
...
def check_iteration(pDialog):
    ...
    if need_to_update:
        ...
        pDialog.close()
        xbmcgui.Dialog().notification('BBTS', 'BBTS is up to date!', xbmcgui.NOTIFICATION_INFO, 5000)
